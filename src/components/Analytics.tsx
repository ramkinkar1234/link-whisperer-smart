  ...
  const handleGetAnalytics = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const mockData: AnalyticsData = {
        ...
        original_url: "https://example.com/very/long/url/that/was/shortened",
        clicks: Math.floor(Math.random() * 500) + 10,
        created_at: Date.now() - (Math.floor(Math.random() * 30) * 24 * 60 * 60 * 1000),
        expiry_time: Date.now() + (30 * 24 * 60 * 60 * 1000),
        password_protected: Math.random() > 0.5
      };
      ...
      toast({
        title: "Error",
        description: "Failed to retrieve analytics. Please check the short code.",
        variant: "destructive"
      });
    } finally {
      setIsLoading(false);
